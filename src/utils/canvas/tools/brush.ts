 ...
 * is called per frame
 */
function applySmoothing({path, maxPointsToSmooth, minDistanceBetweenPoints, smoothingFn}: SmoothingOptions): Path {
...
function bezierSmoothing(path: Path, maxPointsToSmooth: number, minDistanceBetweenPoints: number): Path {
    const newPath = [];
    ...
    while (index < path.length) {
        ...
        const pathLengthToSmooth = Math.min(MAX_BEZIER_INTERPOLATIONS, path.length - index);
        ...
        const pathArr = [];
        for (let i = 0; i < pathLengthToSmooth; i++) 
            pathArr.push(path[index + i])

        
        const normalLineDistance = distance(start, end);
        const numPointsOnLine = Math.floor(normalLineDistance / minDistanceBetweenPoints);

        for (let i = 0; i < numPointsOnLine; i++) {
            const t = (minDistanceBetweenPoints * i) / normalLineDistance;
            newPath.push(NBezier(pathArr, t));

            if (newPath.length == maxPointsToSmooth)
                return newPath;
        }
...
function pointsAtDistances(path: Path, desiredDistances: number[]): Path {
    ...
    for (let i = 0; i < path.length - 1 && currentDistanceIdx < desiredDistances.length; i++) {
        const start = path[i];
        const end = path[i + 1];
        ...
        while (accumulatedDistance <= d && d <= nextDistance) {
            ...
            if (currentDistanceIdx++ == desiredDistances.length)
                return newPath;

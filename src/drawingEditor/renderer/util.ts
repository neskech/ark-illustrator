...
export function constructQuadSixPressureNormal(
  ...
  const sizeTop = settings.getSizeGivenPressure(end.pressure);
  const sizeBottom = settings.getSizeGivenPressure(start.pressure);
...
export function emplaceQuads(
  ...
  for (const p of curve) {
    const size = settings.getSizeGivenPressure(p.pressure);
    const opacity = settings.getOpacityGivenPressure(p.pressure);

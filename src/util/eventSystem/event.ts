...
import type ToolTypes from './eventTypes/toolEvents';
import type CanvasTypes from './eventTypes/canvasEvents';

type ArrayConcat<K extends unknown[], P extends unknown[]> = K extends [infer Head, ...infer Rest]
  ? [Head, ...ArrayConcat<Rest, P>]
  : K extends []
  ? P
  : never;

type MultiArrayConcat<K extends unknown[][]> = K extends [
  infer Head1 extends unknown[],
  infer Head2 extends unknown[],
...
type MakeEventMap<Ts extends object[]> = Ts extends [infer Head]
  ? Head
  : Ts extends [infer Head, ...infer Rest extends object[]]
  ? Head & MakeEventMap<Rest>
  : never;

type AllEvents = MultiArrayConcat<[ToolTypes, CanvasTypes, AppEventTypes]>;

...
export class List<T extends LinearAlgebra> {
  ...
  insertAt(t: T, index: number) {
    ...
    for (let i = this.size; i > stopIndex; i--) this.buffer[i] = this.buffer[i - this.typeSize];

    const end = scaledIndex + this.typeSize;
    for (let i = scaledIndex; i < end; i++) this.buffer[i] = t.values[i - scaledIndex];
  }
  ...
  removeAt(index: number): T {
    ...
    for (let i = scaledIndex; i < end; i++) this.buffer[i] = this.buffer[i + this.typeSize];
  ...
  toString(): string {
    let str = '[';
    for (let i = 0; i < this.size; i++) str += `${this.buffer[i]}${i == this.size - 1 ? '' : ', '}`;
    str += ']';
...
//! VECTOR FUNCTIONS TODO: MAKE THESE GO UNDER A STATIC CLASS TO NAMESPACE THEM
...
export function displacement(from: Float32Vector2, to: Float32Vector2): Float32Vector2 {
  return sub(copy(to), from);
}

export function direction(from: Float32Vector2, to: Float32Vector2): Float32Vector2 {
  return normalize(displacement(from, to));
}

export function distanceAlong(from: Float32Vector2, to: Float32Vector2, t: number): Float32Vector2 {
...
export function midpoint(a: Float32Vector2, b: Float32Vector2): Float32Vector2 {
  return add(scale(displacement(a, b), 0.5), a);
}
...
export function rotateAbout(
  target: Float32Vector2,
  about: Float32Vector2,
  theta: number
): Float32Vector2 {
  const c = copy(target);
  sub(c, about);
